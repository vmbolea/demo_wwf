...
function construirLeyendaImpactos(data) {
  ...
  const tituloOriginal = '<h3>⚠️ Impactos Ambientales</h3>';
  
  // Limpiar solo el contenido interior respetando el título
  // Alternativa: No usar innerHTML = '' que borra todo
  
  // Mejor: Crear un nuevo div para las categorías
  let categoriasContainer = document.getElementById('categoriasImpactosContainer');
  if (!categoriasContainer) {
    // Crear el contenedor si no existe
    categoriasContainer = document.createElement('div');
    categoriasContainer.id = 'categoriasImpactosContainer';
    legendDiv.appendChild(categoriasContainer);
  }
  
  // Limpiar solo el contenedor de categorías, no el título
